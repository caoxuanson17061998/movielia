...
export const CatalogPage = (props: Props) => {
  ...
  switch (props.type) {
    case 'movie':
      title = 'Movies';
      request = (page: number) => tmdbApi.discover('movie', page);
      break;

    case 'tv':
      title = 'TV';
      request = (page: number) => tmdbApi.discover('tv', page);
      break;

    case 'search':
      title = `Search results for <i>${params.get('q')}</i>`;
      request = (page: number) => tmdbApi.search(params.get('q') || '', page);
      break;
    ...
    case 'list':
      ...
      if (title === 'top-rated-tv') {
        request = (page: number) => tmdbApi.getTopRated('tv', page);
      } else if (title === 'top-rated-movies') {
        request = (page: number) => tmdbApi.getTopRated('movie', page);
      }
      break;
