...
export const FilmPage = (props: Props) => {
  ...
  const fetch = async () => {
    const film = await tmdbApi.getDetail(
      props.mediaType,
    ...
    if (film) {
      setFilm(film);
      setCasts(await tmdbApi.getCasts(film.mediaType, film.id));
      setTrailers(await tmdbApi.getTrailers(film.mediaType, film.id));
      setRecommendations(
        await tmdbApi.getRecommendations(film.mediaType, film.id)
      );
    }
  };
